# Rental Management Technical and Product Plan

## 1. Executive Summary

This is an early-stage full-stack MVP for Vietnamese landlords managing approximately 10–50 rooms. It currently supports:
...
The strongest parts are the straightforward technology choice, Supabase RLS, database constraints, and a usable monthly billing workflow.
...
**Recommended direction:** preserve the current architecture and improve it incrementally. Stabilize financial correctness, validation, database invariants, testing, and operational visibility before adding advanced features.
...
The boundaries are not strict. Route handlers contain:
...
There are no structured error codes, centralized error handling, request IDs, or server-side application logging detected.

### Configuration

Environment variables are used for Supabase URL and anon key. The source does not validate required environment variables at startup.
...
No actual SMS, Zalo API, email provider, payment gateway, accounting integration, or background job system exists.
...
This is a client-side message generator. No actual automated notification delivery exists.
...
There is no dedicated reporting module, expense reporting, profit calculation, export, or historical analytics.
...
Keep Supabase and Next.js. Do not add a separate backend unless deployment or scale requirements change materially.
...
Current data migration requirements are **UNKNOWN — requires further investigation** because production data volume, existing users, and deployment history are not available.
